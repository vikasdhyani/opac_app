...
class DeliveryOrdersController < ApplicationController
  ...
  def table
    @order_list = OrderList.all(:membership_no => params[:membership_no])[0]
    render :partial => "table", :locals => { :order_list => @order_list }
  end

  def search_by_slot_and_date
    order_lists = OrderList.all(:membership_no => params[:membership_no], "delivery_schedules.delivery_date" => params[:delivery_date], "delivery_schedules.delivery_slot_id" => params[:slot_id])
